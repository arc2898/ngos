...
static void map_client(Window c){XWindowAttributes a;XGetWindowAttributes(d,c,&a);if(a.override_redirect)return;Window frame=XCreateSimpleWindow(d,root,120,110,720,430,2,col("#8bf7ff"),col("#11182c"));XSelectInput(d,frame,ExposureMask|ButtonPressMask|PointerMotionMask);XAddToSaveSet(d,c);XReparentWindow(d,c,frame,2,28);XMapWindow(d,frame);XMapWindow(d,c);focus=c;XSetInputFocus(d,c,RevertToPointerRoot,CurrentTime);}
int main(void){d=XOpenDisplay(0);if(!d)return 1;int s=DefaultScreen(d);root=RootWindow(d,s);sw=DisplayWidth(d,s);sh=DisplayHeight(d,s);XSelectInput(d,root,SubstructureRedirectMask|SubstructureNotifyMask|KeyPressMask);KeySym keys[]={XK_Return,XK_m,XK_space,XK_q};for(unsigned i=0;i<4;i++)XGrabKey(d,XKeysymToKeycode(d,keys[i]),Mod4Mask,root,1,GrabModeAsync,GrabModeAsync);for(int i=0;i<4;i++)XGrabKey(d,XKeysymToKeycode(d,XK_1+i),Mod4Mask,root,1,GrabModeAsync,GrabModeAsync);shell=XCreateSimpleWindow(d,root,0,0,sw,sh,0,col("#060914"),col("#060914"));XSelectInput(d,shell,ExposureMask|KeyPressMask|PointerMotionMask);XMapWindow(d,shell);XSetInputFocus(d,shell,RevertToPointerRoot,CurrentTime);redraw();for(;;){XEvent e;XNextEvent(d,&e);if(e.type==Expose)redraw();else if(e.type==KeyPress)key(&e.xkey);else if(e.type==MotionNotify){int left=sw/2-210;int n=(e.xmotion.x-left+35)/70;int nh=(n>=0&&n<ICONS)?n:-1;if(nh!=hover){hover=nh;redraw();}}else if(e.type==MapRequest)map_client(e.xmaprequest.window);else if(e.type==DestroyNotify&&focus==e.xdestroywindow.window)focus=0;}}
