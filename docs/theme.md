...
The stable session expects the wallpaper at `/usr/share/backgrounds/ngos/wallpaper.svg` and selects the WhiteSur cursor theme with `XCURSOR_THEME=WhiteSur` and `XCURSOR_SIZE=24`. Run `theme/install-assets.sh` as part of the image-root customization step to install the wallpaper, NGOS logo, cursor metadata, and default Zsh configuration. The source files are not merely preview assets; the session now actively consumes these paths.
